...
/** 内容分区四区枚举（唯一定义源在 taxonomy.ts；此处转发给下游，避免再复制一份词表） */
export { ZONES };
export type { Zone } from "./taxonomy.ts";
...
- ${FUN_RULES_V3}
- "fun_dims": 六维各自的判定值（0 / 0.5 / 1），键名固定为 ${FUN_DIMS.map((d) => d.key).join(" / ")}；fun_score 必须与六维按上面的公式算出的结果一致。
- "zone_reason" / "fun_reason": 各一句话（20 字内）说明依据，供人工抽样核对。
...
- "reason_cn": 简要介绍，110-130 个汉字（硬性要求：少于 ${REASON_MIN} 字或多于 ${REASON_MAX} 字都不合格，约 3 行，写完后数一遍字数确认；**计数按字符个数**——英文、数字、空格、标点各算一个字符）。面向第一次听说它的读者，用他们能懂的话讲清它是什么、解决什么问题、具体能做到什么。可以出现专业概念，但要让外行也读得懂。一段连贯文字，不要用①②③等序号编号，不要重复 summary_cn 的内容。**必须把话说完，以句末标点（。！？）收尾**——结尾停在半个词、半个列举或逗号上都不合格（九轮 T4 实测：全库 573 张的结尾就断在半句上，读起来像被裁掉一截）。
- ${DETAIL_SPEC}
${QIANREN_SPEC}
