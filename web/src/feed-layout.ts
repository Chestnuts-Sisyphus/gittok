 ...
 *  2026-09-23 三次取值 500 → 550 → 536 → **460**。最后一次门限由**可读区间**定而非「最宽卡能放多少字」：
 *  中文行长可读区间 22–38 汉字（Bringhurst 45–75 拉丁字符 ÷ 2；Unicode TR11：全角 1em／半角 1/2em；
 *  WCAG 2.2 SC 1.4.8 只给上限「80 chars (40 if CJK)」）。460px 内容宽 391px ÷ 16.66 = 23.5 → **23 汉字**（区间下沿 22 之上）。
 *  门限抬高会让更多视口掉进「单列 + 两侧大留白」（栗子 09-23 截图那条）。
 *  CSS 的轨道下限写的是 `min(460px, 100%)`——容器窄于 460 时轨道退成容器宽、仍算 1 列，
 *  与本函数 `Math.max(1, …)` 同值，故两边在全部实测宽度上一致（517/573/673/873/1048/1105/1321/1602 逐档比对过）。 */
export const FEED_COL_MIN = 460;
